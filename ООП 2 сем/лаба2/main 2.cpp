...
int main() {
    ...
    for (size_t i = 0; i < park.size(); ++i) {
        ...
        std::cout << "Ускорили " << park[i]?? пока не реализован вывод марки, лучше просто показать действие.
    // Можно вывести информацию после ускорения каждого, но по заданию достаточно итога после всех действий.
    // Для наглядности выведем промежуточное состояние после ускорения:
